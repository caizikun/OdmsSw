...
class CsantecTls
{
    ...
    public bool ConnectByGpib(int _gpibAddr)
    {
        ...
        try
        {
            ...
            if (strResponse.IndexOf("SANTEC") < 0)
                throw new ApplicationException();
    ...
    public void SetWavelen(double _wl)
    {
        ...
        {
            string strCmd = ":WAV ";
            strCmd += Math.Round(_wl, 3).ToString();
            m_gpibDev.Write(strCmd);
    ...
    public List<double> GetWavelenLog()
    {
        ...
        try
        {
            ...
            int nDigitNum = 0;
            resp = m_gpibDev.ReadByteArray(2);
            nDigitNum = Convert.ToInt32(Encoding.Default.GetString(resp, 1, 1));


            //Read size of binary data block
            int nDataBlockSize = 0; //Size of binary data block
            int nDataPointCnt = 0; //Number of data.
            resp = m_gpibDev.ReadByteArray(nDigitNum);
            string strTempBuf = "";
            for( int i = 0 ; i < nDigitNum; i++)
            {
                strTempBuf += Encoding.Default.GetString(resp, i, 1);
            }
            nDataBlockSize = Convert.ToInt32(strTempBuf);
            nDataPointCnt = nDataBlockSize / 4;
            ...
            //Data를 읽는다.
            resp = null;
            resp = m_gpibDev.ReadByteArray(nDataBlockSize);
            
            
            //Byte 단위의 데이터를 int 타입으로 변환해준다.!!
            int[] targetBufs = new int[nDataPointCnt];
            IntPtr ptrTarget = Marshal.AllocHGlobal(4);
            int nSourBufIdx = 0;
            for(int i = 0; i < nDataPointCnt; i++ )
            {
                Marshal.Copy(resp, nSourBufIdx, ptrTarget, 4);
                int[] tempIntArr = new int[1];
                Marshal.Copy(ptrTarget, tempIntArr, 0, 1);
                targetBufs[i] = tempIntArr[0];
                nSourBufIdx += 4;
            }
            ...
            for(int i = 0 ; i < nDataPointCnt ; i++ )
            {
                wavelen = Math.Round((double)(targetBufs[i]/10000), 3);
                retList.Add(wavelen);
            }
