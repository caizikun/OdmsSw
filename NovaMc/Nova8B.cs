...
namespace Free302.TnM.Device
{
	...
	/// Nova 8축 MC
	/// </summary>
	public class Nova8B : McBase<StageBase>
	{
		...
		public UInt16 Nova8B_is_error(UInt16 id, McAxis mcAxis)
		{
			...
			NmcAxis axis = getNmcAxis(mcAxis);
		...
		public UInt16 Nova8B_flag_error(UInt16 id, McAxis mcAxis)
		{
			...
			errorCode = (UInt16)MC8000P.Nmc_ReadReg0(id, icNo);
		...
		public UInt16 Nova8B_error(UInt16 id, McAxis mcAxis)
		{
			...
			// RR2 레지스터를 읽어서 에러코드 값을 리턴
			UInt16 errorCode = (UInt16)MC8000P.Nmc_ReadReg2(id, icNo, (AXIS)Enum.Parse(typeof(AXIS), axis.ToString().ToUpper()));
		...
		//생성자
		public Nova8B() : this(McId.None) { }

		public Nova8B(McId id, string name = DefaultName)
			: base(id, name)
		{
			//PmcAddress
		...
		private NmcAxis getNmcAxis(McAxis mcAxis)
		{
		...
		override public DeviceConfig buildConfig()
		{
			...
			//PmcAddress
			mConfig.Add("PmcAddress", this.NovaAddress);
		...
		static void openAddress(ushort address)
		{
			...
			if (counter == 0)
			{
				...
				// 2017.06.20 Jeon
				var status = Nova8BWrap.MC8000P.Nmc_Open(address, false);
				MC8000P.Nmc_Reset(address, 0);
				MC8000P.Nmc_Reset(address, 1);

				MC8000P.Nmc_WriteReg1(address, 0, AXIS.ALL, 0x0000);
				MC8000P.Nmc_WriteReg1(address, 1, AXIS.ALL, 0x0000);

				// HLMT+, HLMT- 설정
				MC8000P.Nmc_WriteReg2(address, 0, AXIS.ALL, 0x0018);
				MC8000P.Nmc_WriteReg2(address, 1, AXIS.ALL, 0x0018);

				// WR3 입력 신호의 전 필터 설정 및 입력 신호 지연 010 => 512μs -> 0.5msec
				MC8000P.Nmc_WriteReg3(address, 0, AXIS.ALL, 0x4F00);
				MC8000P.Nmc_WriteReg3(address, 1, AXIS.ALL, 0x4F00);

				// 일반 출력신호 값 설정
				MC8000P.Nmc_WriteReg4(address, 0, 0x0000);
				MC8000P.Nmc_WriteReg4(address, 1, 0x0000);

				// 범용 출력신호 활성여부 설정
				//MC8000P.Nmc_WriteReg5(address, 0, 0x0000);
				//MC8000P.Nmc_WriteReg5(address, 1, 0x0000);
				MC8000P.Nmc_WriteReg5(address, 0, 0xffff);
				MC8000P.Nmc_WriteReg5(address, 1, 0xffff);

				// 
				MC8000P.Nmc_AccOfst(address, 0, AXIS.ALL, 0);
				MC8000P.Nmc_AccOfst(address, 1, AXIS.ALL, 0);
		...
		protected override void performOpen()
		{
			...
			foreach (McAxis mcAxis in mAxisMap.Keys)
			{
                ...
                //var act = readPosition(mcAxis);
                //if (act != stage.LastPositionμm) throw new Exception();
			}
		}
		/// <summary>
		/// 열렸는지 검사
		...
		private void setDriveMode(McAxis mcAxis)
		{
			...
			try
			{
				//Nova8BWrap.MC8000P.Nmc_WriteReg2(1, this.mPmcAddress, (Nova8BWrap.Ac)axis, (UInt16)wData);	
				//Nova8BWrap.MC8000P.Nmc_WriteReg2(this.mPmcAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), axis.ToString()), (UInt16)wData);
				//Nova8BWrap.MC8000P.Nmc_WriteReg2(this.mNovaAddress, icNo, AXIS.ALL, (UInt16)wData);
				// Jeon 2017.0905
				Nova8BWrap.MC8000P.Nmc_WriteReg2(this.mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), axis.ToString().ToUpper()), (UInt16)wData);
				status = Nova8BWrap.MMC_OK;
			}
			catch (Exception ex)
			{
				status = Nova8BWrap.MMC_ERROR;
			}
			//checkPmcStatus(PmcExceptionId.setMode, status, mPmcAddress, axis);
		}
		...
		private void setHwLimit()
		{
			...
			//ushort status = Pmc4BWrap.pmc4bpci_wwr2(this.mPmcAddress, (UInt16)NovaMcAxis.All, wData);
			// 2017.06.23 Jeon
			// HLMT+, HLMT- 설정
			MC8000P.Nmc_WriteReg2(this.mNovaAddress, 0, AXIS.ALL, wData);
			MC8000P.Nmc_WriteReg2(this.mNovaAddress, 1, AXIS.ALL, wData);
		...
		private void setSensorConfig(McAxis mcAxis, McSensorConfig sensor)
		{
			...
			{
				// 2017.06.20
				enableFlag |= Nova8BWrap.WR1_IN0_E;
				levelFlag |= Nova8BWrap.WR1_IN0_L;//ActiveHigh
			}
			if (sensor.HasFlag(McSensorConfig.ORG))
			{
				// 2017.06.20
				enableFlag |= Nova8BWrap.WR1_IN1_E;
				levelFlag |= Nova8BWrap.WR1_IN1_L;//ActiveHigh
			}
			*/
			NmcAxis axis = getNmcAxis(mcAxis);
			UInt16 icNo = (UInt16)(NovaMcIcNo)Enum.Parse(typeof(NovaMcIcNo), mcAxis.ToString());
            ...
            // jeon 2017.0905
            //flag = (uint)stage.DirPulseLevel | (uint)stage.DrivePulseMode | (uint)stage.SensorActiveLevel | WR2_LMT_HLMTP | WR2_LMT_HLMTM;
            ...
            // 2017.06.22 Jeon
            Nova8BWrap.MC8000P.Nmc_WriteReg2(this.mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), axis.ToString().ToUpper()), (UInt16)flag);
		...
		{
			UInt16 icNo = (UInt16)(NovaMcIcNo)Enum.Parse(typeof(NovaMcIcNo), mcAxis.ToString());
			NmcAxis novaAxis = getNmcAxis(mcAxis);

			// 논리 위치 읽어 옴
			int pos = MC8000P.Nmc_ReadLp(this.mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()));
		...
		private void writePosition(McAxis mcAxis, double newPosition)
		{
			...
			// 2017.06.23 Jeon Position write
			UInt16 icNo = (UInt16)(NovaMcIcNo)Enum.Parse(typeof(NovaMcIcNo), mcAxis.ToString());
			NmcAxis novaAxis = getNmcAxis(mcAxis);
			MC8000P.Nmc_Lp(mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()), internalPosition);
			//MC8000P.Nmc_Command(mPmcAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), pmcAxis.ToString().ToUpper()), CMD.);
		...
		private bool readIsDrive(McAxis axis)
		{
			...
			NmcAxis novaAxis = getNmcAxis(axis);
			UInt16 status = (UInt16)MC8000P.Nmc_GetDriveStatus(this.mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()));
		...
		private bool readIsStop(McAxis mcAxis)
		{
			...
			return !((MC8000P.Nmc_GetDriveStatus(this.mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper())) >= (UInt16)NovaMcErrorStatus.TRUE));
		...
		private bool readFlagLMTP(McAxis mcAxis)
		{
			...
			return (MC8000P.Nmc_ReadReg2(this.mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper())) & 0x0004) >= (UInt16)NovaMcErrorStatus.TRUE;
			// Jeon 2017.09.01
		...
		private bool readFlagLMTN(McAxis mcAxis)
		{
			...
			UInt16 icNo = (UInt16)(NovaMcIcNo)Enum.Parse(typeof(NovaMcIcNo), mcAxis.ToString());
			NmcAxis novaAxis = getNmcAxis(mcAxis);
			return (MC8000P.Nmc_ReadReg2(this.mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString())) & 0x0008) >= (UInt16)NovaMcErrorStatus.TRUE;
		}
		...
		public override void stop(McAxis mcAxis = McAxis.All)
		{
			...
			MC8000P.Nmc_Command(mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), x.ToString().ToUpper().ToUpper()), CMD.CMD_STOP_SUDDEN);
		}
        ...
        private void writeMove(McMotionParam param)
        {
            ...
            NmcAxis novaAxis = getNmcAxis(param.axis);

            checkAxis(novaAxis);

            // Ic chip Number Check
            UInt16 icNo = (UInt16)(NovaMcIcNo)Enum.Parse(typeof(NovaMcIcNo), param.axis.ToString());
            ...
            int R = PmcHwInfo.R(stage.PulseDivider);//400000
                                                    ...
                                                    // 2017.06.22
            MC8000P.Nmc_Range(mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()), R);
            ...
            //move
            //ushort status = ushort.MaxValue;
            // Pmc4BWrap.pmc4bpci_pls_smove(mPmcAddress, (ushort)pmcAxis, (int)dx0, (short)param.speed, NovaMmcAccel.ac, NovaMmcAccel.aac, NovaMmcAccel.dc, NovaMmcAccel.ddc);
            // 2017.06.21 Jeon
            //----------------------------------------------------
            //MC8000P.Nmc_WriteReg1(mNovaAddress, (int)IC.B, AXIS.ALL, 0x0000);
            //MC8000P.Nmc_WriteReg2(mNovaAddress, (int)IC.B, AXIS.ALL, 0x0018);
            //MC8000P.Nmc_WriteReg2(mNovaAddress, 0, AXIS.ALL, 0x0018);
            //MC8000P.Nmc_WriteReg2(mNovaAddress, 1, AXIS.ALL, 0x0018);
            //====================================================
            setDriveMode(param.axis);

            //----------------------------------------------------
            //MC8000P.Nmc_WriteReg3(mNovaAddress, (int)IC.B, AXIS.ALL, 0x4F00);
            //MC8000P.Nmc_WriteReg4(mNovaAddress, (int)IC.B, 0x0000);
            //MC8000P.Nmc_WriteReg5(mNovaAddress, (int)IC.B, 0x0000);

            //MC8000P.Nmc_AccOfst(mNovaAddress, (int)IC.B, AXIS.ALL, 0);
            //MC8000P.Nmc_Range(mNovaAddress, (int)IC.B, AXIS.ALL, 800000);
            //====================================================

            MC8000P.Nmc_Jerk(mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()), NovaMmcAccel.aac);
            MC8000P.Nmc_DJerk(mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()), NovaMmcAccel.ddc);

            MC8000P.Nmc_Acc(mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()), NovaMmcAccel.ac);
            MC8000P.Nmc_Dec(mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()), NovaMmcAccel.dc);


            MC8000P.Nmc_StartSpd(mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()), 100);
            MC8000P.Nmc_Speed(mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()), (UInt16)param.speed);

            //----------------------------------------------------
            //Nova8BWrap.MC8000P.Nmc_Pulse(mNovaAddress, 0, AXIS.ALL, (int)dx0);
            //Nova8BWrap.MC8000P.Nmc_Pulse(mPmcAddress, (int)IC.B, AXIS.ALL, (int)dx0);
            //MC8000P.Nmc_Command(mNovaAddress, 0, (AXIS)pmcAxis, CMD.CMD_F_DRV_M); // 명령:33 X 축 '-'방향 정량 드라이브
            //MC8000P.Nmc_Command(mNovaAddress, 0, AXIS.ALL, CMD.CMD_F_DRV_M); // 명령:33 X 축 '-'방향 정량 드라이브
            //====================================================

            // 2019.01.25 by DrBAE ~ 좌표계 기본원칙에 따라 불필요
            //실제축 & 논리축간 좌표변환
            //var trans = stage.OriginPolarity == StageOriginPolarity.PosLimit;
            var positiveDir = dx0 > 0;
            //dx0 = positiveDir ? dx0 : -dx0;
            dx0 = Math.Abs(dx0);

            if (positiveDir)
			{
				MC8000P.Nmc_Pulse(mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()), (int)dx0);
				MC8000P.Nmc_Command(mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()), CMD.CMD_F_DRV_P); // 명령:33 X 축 '+'방향 정량 드라이브
			}
			else
			{
				MC8000P.Nmc_Pulse(mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()), (int)dx0);
				MC8000P.Nmc_Command(mNovaAddress, icNo, (AXIS)Enum.Parse(typeof(AXIS), novaAxis.ToString().ToUpper()), CMD.CMD_F_DRV_M); // 명령:34 X 축 '-'방향 정량 드라이브
			}
		...
		public override void moveToStrokeCenter(McAxis mcAxis = McAxis.All)
		{
            ...
            foreach (var axis in dicEff.Keys)
            {
                StageBase stage = getStage(axis);
                ...
                //while(isMoving(param.axis))
                //{
                //    var p = readPosition(param.axis);
                //    pos.Add(p);
                //    Thread.Sleep(100);
                //}
                //var a = pos.Max();
