...
namespace Neon.Aligner
{
    ...
    public class Agilent8164
    {
        ...
        public bool isN7744 => Query("*IDN?").Contains("7744");
		...
		public void SetSwTrigger(int _slotNo, bool onOff)
		{
			try
			{
				...
				//Sets the starting point of the sweep
				string strCommand = "";
				strCommand += "INIT" + _slotNo.ToString() + ":cont " + (onOff ? "1" : "0");
		...
		//        byteResponse = m_gpibDev.ReadByteArray(7);  //읽어야할 데이터 개수 포함.


		//        //읽어야할 데이터 갯수를 구해야 하지만... 계산으로 한다.  
		//        //Dim nDigitNum As Integer = 0
		//        //nDigitNum = Val(System.Text.Encoding.Default.GetString(byteResponse, 1, 1))

		//        //Read size of binary data block
		//        //byteResponse = m_gpibDev.ReadByteArray(nDigitNum)

		//        //Dim strTempBuf As String = ""
		//        //Dim nBinaryDataBlockSize As Integer = 0  '//Size of binary data block
		//        //Dim i As Integer = 0
		//        //For i = 0 To nDigitNum - 1
		//        //    strTempBuf = strTempBuf + System.Text.Encoding.Default.GetString(byteResponse, i, 1)
		//        //Next
		...
		public List<double> GetSourLambdaLog(int _slotNo, int _chnlNo)
		{
			try
			{
				...
				m_gpibDev.Write($"SOUR{_slotNo}:READ:DATA? LLOG");
				var bytes = m_gpibDev.ReadByteArray();
			...
			finally { Monitor.Exit(m_gpibDev); }
		}
		...
		public List<double> GetSensLog(int _slotNo, int _chnlNo)
        {
            ...
            try
            {
				...
				for (int i = 0; i < 10; i++)
                {
                    strResp = Query($"SENS{_slotNo}:CHAN{_chnlNo}:FUNC:STATE?");
                    if (strResp.Contains("COMPLETE")) break;

					if (i >= 9)
					{
						int dp = 0;
                        double avg = 0;
						GetSensLogParam(_slotNo, ref dp, ref avg);
                        var msg = $"[Agilent8164.GetSensLog()] {_slotNo}.{_chnlNo} : DP=<{dp}>, ATime=<{avg}>, <{strResp}>";
                        logError(msg);
                ...
                // -----   Logging data Header -----
                m_gpibDev.Write($"SENS{_slotNo}:CHAN{_chnlNo}:FUNC:RES?");
                ...
                int nDigitNum = 0;
                nDigitNum = Convert.ToInt32(Encoding.Default.GetString(byteResponse, 1, 1));
                ...
                {
                    strTempBuf += Encoding.Default.GetString(byteResponse, i, 1);
                }
        ...
        {
            System.IO.StreamWriter sw = new System.IO.StreamWriter(@"log\device_log.txt", true);
