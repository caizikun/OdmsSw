...
namespace Neon.Aligner
{
    ...
    public class Agilent8169 : IpolController
    {
        ...
        {
            if (false == m_connected) throw new ApplicationException();

            //Send Command
            _pos = Math.Round(_pos, 2);
            string strCommand = "POS:POL " + Convert.ToString(_pos);
            m_gpibDev.Write(strCommand);
            WaitForGpibIdle();

            var actual = Math.Round(GetPolFilterPos(), 2);
            if (Math.Abs(_pos - actual) > 0.01) throw new Exception($"SetPolFilterPos(): exp={_pos} != act={actual}");
        }
        ...
        public void SetHalfRetarderPos(double _pos)
        {
            ...
            var actual = Math.Round(GetHalfRetarderPos(), 2);
            if (Math.Abs(_pos - actual) > 0.01) throw new Exception($"SetHalfRetarderPos(): exp={_pos} != act={actual}");
        }
        ...
        public void SetQuarRetarderPos(double _pos)
        {
            ...
            var actual = Math.Round(GetQuarRetarderPos(), 2);
            if (Math.Abs(_pos - actual) > 0.01) throw new Exception($"SetQuarRetarderPos(): exp={_pos} != act={actual}");
        }
        ...
        /// <summary>
        /// set Polarization Controller as LH(Left Hand) circular
        /// </summary>
        /// <param name="dbPolPos"></param>                
        public void SetToLHcircular(double dbPolPos)
        {
            SetPolFilterPos(dbPolPos);
            SetQuarRetarderPos(dbPolPos - 45);
            SetHalfRetarderPos(dbPolPos);
